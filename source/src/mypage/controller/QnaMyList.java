...
public class QnaMyList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		IQnaService myQnaService = QnaServiceImpl.getInstance();
		
		//pagination
		int currentPage = 1;
		currentPage = request.getParameter("pageNo") == null ? 1 : Integer.parseInt(request.getParameter("pageNo"));
		int count = myQnaService.countList();  //--->쿼리수정필요해요 board테이블 부재 / adminQnA에 맞는 테이블 리스트카운트 값을 불러와야 합니당!!
		int perPage = 5;
		int perList = 10;
		...
		//목록 조회
		List<QnaVO> myQnaList = myQnaService.selectAllMember(map); //전체 조회하는 것
		
		request.setAttribute("adminQnaList", myQnaList);
